...
def get_fluid_domain_modifier(obj):
    """
    Find the fluid domain modifier on the given object.
    
    Args:
        obj: Blender object to check
        
    Returns:
        Fluid domain modifier if found, None otherwise
    """
    if not obj:
        return None
...
def evaluated_mesh_vertex_count(context, domain_obj, frame):
    """
    Get the vertex count of the evaluated mesh at a specific frame.
    
    Args:
        context: Blender context
        domain_obj: Fluid domain object
        frame: Frame number to evaluate
        
    Returns:
        Number of vertices in the evaluated mesh
    """
    scene = context.scene
...
class OBJECT_OT_fluid_cache_to_mesh_sequence(bpy.types.Operator):
    """
    Convert a baked liquid fluid mesh cache into per-frame mesh objects.
    
    This operator creates individual mesh objects for each frame in the fluid cache,
    with proper keyframe animation to show/hide the meshes at the correct frames.
    """
...
def draw_fluid_cache_menu(self, context):
    """
    Add the fluid cache to mesh sequence operator to the object context menu.
